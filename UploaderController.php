<?php
    ...
    use frontend\models\ProductInfo;
    use frontend\models\Category;
    use frontend\models\ProdImages;
...
class UploaderController extends Controller {
    ...
    function actionDelete()
    {
        ...
        else
        {
            ...
            $image = Yii::$app->db->createCommand($sql)->bindParam(':id', $id)->bindValue(':id_user', Yii::$app->session->getHasSessionId())->queryOne();
            if(!$image)
            {
                return false;
            }
            ...
            $this->deleteFile(Yii::$app->session->getHasSessionId(), $img_path_key, $image['id_prod'], $image['img_url']);
            ...
            if( $image['is_main']==1 )
            {
                ...
                Yii::$app->db->createCommand($sql)->bindValue(':id_user', Yii::$app->session->getHasSessionId())->execute();
                $sql = 'SELECT id FROM {{%prod_temp_images}} WHERE id_session=:id_user AND is_main=1';
                $main_image = Yii::$app->db->createCommand($sql)->bindValue(':id_user', Yii::$app->session->getHasSessionId())->queryOne();
            }

            return json_encode(array('ok'=>'ok', 'id_main'=>$main_image['id']));
        }
    }
    ...
    function actionSetFav()
    {
        ...
        else
        {
            ...
            Yii::$app->db->createCommand($sql)->bindParam(':id',$id)->bindValue(':id_user', Yii::$app->session->getHasSessionId())->execute();
        }
    ...
    function deleteFile($id_user, $img_path_key, $id_item, $img_url)
    {
        ...
        foreach($img_dir as $k=>&$dir)
        {
            ...
            if( is_array($dir) )
            {
                foreach($dir as &$path)
                {
                    $path = Yii::getAlias('@frontend/web/').strtr($path,array('{id_user}'=>$id_user, '{id_item}'=>$id_item));
            ...
            {
                $dir = Yii::getAlias('@frontend/web/').strtr($dir,array('{id_user}'=>$id_user, '{id_item}'=>$id_item));
...

        {
            ...
            if( $f_size>MAX_SIZE )
            {
                ...
                else
                {
                    ...
                    $info['url'] = Url::to($tmpFolder.$fileName,true);
    ...
    function actionChangeUserLogo()
    {
        ...
        $image = Yii::$app->db->createCommand($sql)->bindParam(':user', Yii::$app->user->id)->queryOne();

        if(!$image)
        {
            $image['date_create'] = Yii::$app->user->identity->time_reg;
            $image['img_url'] = Yii::$app->user->identity->img_url;
        ...
        $this->createFolders($img_dir_upload, Yii::$app->user->identity->time_reg);
...

        {
            ...
            if( $f_size>MAX_SIZE )
            {
                ...
                else
                {
                    ...
                    {
                        return json_encode(array('err'=>CLib::param('SOMETHING_WRONG'), 'data'=>array($x,$y)));
                    }
                    ...
                    $this->deleteFile(Yii::$app->user->id, User::IMG_PATH, Yii::$app->user->identity->time_reg, Yii::$app->user->identity->img_url );

                    Yii::$app->user->identity->img_url = $filenameNew;
                    ...
                    $info['ok'] = true;
                    $info['url'] = Url::to($img_dir_upload['preview']['200'].$filenameNew,true);
